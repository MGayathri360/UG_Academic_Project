...
new_client_dns () {
        echo "Select a DNS server for the client/user:"
...
new_client_setup () {
        ...
        # Don't break the configuration in case the address space is full
        if [[ "$octet" -eq 255 ]]; then
                echo "253 clients are already configured.internal subnet is full!"
                exit
        fi
        key=$(wg genkey)
        psk=$(wg genpsk)
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
        ...
        echo
        echo "What port should we listen to?"
        ...
        echo
        echo "installation is ready to begin."
